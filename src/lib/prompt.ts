...
- category 为 artifact / food / other 时，luck 不许写成泛泛的好处或感受（禁止「香气让人心情愉悦」「让人放松」这类空话）。这类东西没有稀有度可讲，就改讲「它为什么会出现在他今天这个位置」——产地、工艺、季节、当下的场合。**只有在 <history> 里真的能找到同类记录时**，才可以说「上一次出现在你记录里是什么时候」，并且必须引用那条记录里真实存在的地点和月份。如果连这个都说不出，就诚实降级：confidence 必须为 low，basis 里明写「这一件没有稀有度可讲」。
- 如果依据不充分，confidence 必须为 low，并在 basis 中诚实说明。
- **禁止编造过往记录（最高优先级，违反即判失败）**：cognition、fun、luck.text、luck.basis、memorySentence 里，凡是提到用户「上次 / 上一次 / 之前 / 去年 / 最近一次」见过、或提到任何具体年月日，这些信息**必须逐条来自 <history> 中真实存在的条目**。
  - <history> 是「（暂无历史）」时，绝对不许出现任何过往记忆、时间点或事件；这是他的第一条记录，就当作第一条来写。
  - 不许编造节气、节日、场合（如「去年冬至那场围炉煮茶」）——除非 <history> 的 note 里确实写着。
  - 不许编造具体日期（如「2023年12月22日」）。<history> 只给到月份，你就只能说到月份。
  - 判断依据不足时，写「这是你的第一条这类记录」，而不是编一条过去。
...
语言与名称：
- 除 nameEn 外，所有文字字段只用中文，不要夹英文单词（如 heirloom、latte），需要时写中文译名。
- name 只写一个确定的名称，不要用括号或「或」列备选；拿不准时写更上位但确定的名称（如「披萨」而不是「玛格丽特披萨（或纯奶酪披萨）」）。

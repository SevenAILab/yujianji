...
export function useUniverseNodes(): { nodes: UniverseNode[]; loading: boolean; sample: boolean } {
  ...
  const items = useLiveQuery(() => db.items.filter((item) => !item.isSeed && item.ai?.verdict === "first").toArray(), []);
  ...
  const built = useMemo(
    () => (items && moments && manifest ? buildUniverseNodes({ items, moments, manifest, timeZone }) : null),
    [items, moments, manifest, timeZone],
  );

...
export default function ItemDetail({ id }: { id: string }) {
  ...
  const item = useLiveQuery(
    () => (seedReady ? db.items.get(id).then((stored) => stored ?? null) : Promise.resolve(undefined as Item | null | undefined)),
    [id, seedReady],
  );
  ...
  async function redevelop() {
    if (LOCAL_ONLY) { setActionError("离线模式不发送照片，云端 AI 已关闭。"); return; }
        ...
        <div className="content-stack">
          {item.ai ? (
            ...
            <section className="verdict-card">
              ...
              <button className="secondary-action" style={{ marginTop: 14 }} onClick={() => void redevelop()} disabled={redeveloping || LOCAL_ONLY}>
                {LOCAL_ONLY ? "离线模式 · AI 已关闭" : redeveloping ? "正在重新显影…" : "重新显影"}
              </button>
            </section>
          )}
